CREATE OR REPLACE FUNCTION OPERACION.f_sum_materialxetapa( a_codot in number, a_codpre in number, a_idubi in number, a_codeta in number)  RETURN NUMBER IS
   ...
   --  Costo de Materiales de AUTORIZACIONES DE PEX
      ...
      select sum(cosins * canins) into l_costo
         ...
         where codpre = a_codpre and
               ...
               codeta = a_codeta;--select codeta from etapa where descripcion like 'Permisos%';
