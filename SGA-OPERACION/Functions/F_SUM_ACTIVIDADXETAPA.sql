CREATE OR REPLACE FUNCTION OPERACION.f_sum_actividadxetapa( a_codot in number, a_codpre in number, a_idubi in number, a_codeta in number, a_moneda in char, a_tipo in number)  RETURN NUMBER IS
   ...
   --  Costo de Materiales de AUTORIZACIONES DE PEX
      ...
      select sum(cosins * canins) into l_costo
         ...
         where preubietaact.codact = actividad.codact and
		 	   codpre = a_codpre and
               idubi = a_idubi and
               codeta in (22,201) and
               codeta = a_codeta and
			   actividad.moneda = a_moneda and
 			   actividad.espermiso = a_tipo;--select codeta from etapa where descripcion like 'Permisos%';
