CREATE OR REPLACE PACKAGE BODY OPERACION.PQ_JANUS_RECONEXION IS
  /****************************************************************************************************
     NAME:       PQ_JANUS_SUSPENSION
     PURPOSE:    Ejecutar proceso DE RECONEXION EN JANUS
  
     REVISIONS:
     Ver        Date        Author           Solicitado por                  Description
     ---------  ----------  ---------------  --------------                  ----------------------
     1.0        20/02/2014  Eustaquio Gibaja Christian Riquelme            Ejecutar proceso DE RECONEXION EN JANUS
  ***************************************************************************************************/

  PROCEDURE reconexion_padre(p_idtareawf tareawf.idtareawf%TYPE,
  ...
  BEGIN
    ...
    IF NOT pq_int_telefonia.es_masivo_hfc() THEN
      RAISE_APPLICATION_ERROR(-20000,
                              $$PLSQL_UNIT || '.RECONEXION_PADRE: NO ES MASIVO');
    END IF;
  ...
  BEGIN
    operacion.pq_int_telefonia.g_operacion := 'RECONEXION';
  ...
  PROCEDURE crea_int_plataforma_bscs IS
    C_RECONEXION CONSTANT PLS_INTEGER := 3;
  ...
  BEGIN
    ...
    l_int_bscs.action_id := C_RECONEXION;
    ...
    operacion.pq_janus.g_tx_bscs   := operacion.pq_janus.set_tx_bscs(C_RECONEXION);
    operacion.pq_janus.g_action_id := C_RECONEXION;
