...
LOOP
  ...
  IF v_contador=1 THEN 
      ...
      VALUES(v_cod_tipopedd,'ESTADO_LINEA', 'EST_LINEA');
      
      ---detalle               
      insert into operacion.opedd (IDOPEDD, CODIGOC, CODIGON, DESCRIPCION, ABREVIACION, TIPOPEDD, CODIGON_AUX)
      values ((SELECT MAX(IDOPEDD)+1 FROM OPERACION.OPEDD), '', 1, 'ESTADO DE LA LINEA', 'EST_LINEA', v_cod_tipopedd, null);
      
      ---detalle               
      insert into operacion.opedd (IDOPEDD, CODIGOC, CODIGON, DESCRIPCION, ABREVIACION, TIPOPEDD, CODIGON_AUX)
      values ((SELECT MAX(IDOPEDD)+1 FROM OPERACION.OPEDD), '', 2, 'ESTADO DE LA LINEA', 'EST_LINEA', v_cod_tipopedd, null);
