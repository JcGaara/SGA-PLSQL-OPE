...
LOOP
  ...
  IF v_contador=1 THEN 
      ...
      VALUES(v_cod_tipopedd,'Inversion - Tipo de División', 'AR_TIP_DIV');
      
      ---detalle               
      insert into operacion.opedd (IDOPEDD, CODIGOC, CODIGON, DESCRIPCION, ABREVIACION, TIPOPEDD, CODIGON_AUX)
      values ((SELECT MAX(IDOPEDD)+1 FROM OPERACION.OPEDD), '1', 1, 'Sin División', 'SDIV', v_cod_tipopedd, null);

      insert into operacion.opedd (IDOPEDD, CODIGOC, CODIGON, DESCRIPCION, ABREVIACION, TIPOPEDD, CODIGON_AUX)
      values ((SELECT MAX(IDOPEDD)+1 FROM OPERACION.OPEDD), '2', 2, 'Con División', 'CDIV', v_cod_tipopedd, null); 
