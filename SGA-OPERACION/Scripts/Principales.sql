...
select vc.tipper DATAC_TIPO_PERSONA,
       ...
       (select tipsrv from vtatabslcfac where numslc = iso.numslc ) DATAC_TIPSRV,
       (select dsctipsrv 
          from tystipsrv 
         where tipsrv = (select tipsrv from vtatabslcfac 
                          where numslc = iso.numslc )) DATAV_DESCTIPSRV,
       (select v.idcampanha from vtatabslcfac v where v.numslc = iso.numslc)  DATAN_CODCAMP, 
       (select c.DESCRIPCION 
          from campanha c, vtatabslcfac v 
         where c.IDCAMPANHA = v.idcampanha 
           and v.numslc = iso.numslc )  DATAV_DESCAMP, 
       (select PLAZO_SRV from vtatabslcfac where numslc = iso.numslc ) DATAN_CODPLAZO,
       (select tc.descripcion 
          from tipo_calificacion tc, vtatabslcfac v 
         where v.numslc = iso.numslc
           and v.plazo_srv = tc.codigo(+) and tc.ESTADO = '1'  and tc.tipo = 'C') DATAV_DESCPLAZO,
       ...
       iso.codsrv DATAC_CODSRV,
       (select dscsrv from tystabsrv where codsrv = iso.codsrv) DATAV_SERVICIO,
       (select s.IDSERVICIO_SISACT from sales.servicio_sisact s where s.codsrv = iso.codsrv) DATAV_EQ_SERV_SISACT,
       case
         when ipo.flgprinc = 1 and ipo.codequcom is null then
          ipb.descripcion
       end DATAV_DESCPLAN,
      'Principal' DATAV_TIPOSERVICIO,
      ...
      (select descripcion from estinssrv where  estinssrv = iso.estinssrv) DATAV_DESCESTSERV,
       ...
       iso.codinssrv DATAN_CODINSSRV, 
       ipo.pid DATAN_PID,
       null DATAN_IDMARCAEQUIPO,
       null DATAV_MARCAEQUIPO,
       null DATAC_CODTIPEQU,
       null DATAN_TIPEQU,
       null DATAV_TIPO_EQUIPO,
       null DATAV_EQU_TIPO,
       null DATAC_COD_EQUIPO,
       null DATAV_MODELO_EQUIPO,
       'Servicio' DATAV_TIPO,
       iso.numero DATAV_NUMERO,
       ...
       case
         when ipo.flgprinc = 1 then
          ipb.montocr
       end DATAN_CARGOFIJO,
       ipb.cantidad DATAN_CANTIDAD,
       (select decode(nvl(publicar, 0), 1, 'Si', 0, 'No')
          from numtel
         where numero = iso.numero) DATAC_PUBLICAR,
      ...
      (SELECT vf.nomemail FROM marketing.vtaafilrecemail vf, VTATABPRECON vp, vtatabslcfac v 
       ...
       (select codest FROM v_ubicaciones v WHERE vc.codubi = v.codubi) DATAV_IDDEP_DIRCLI,
       (select codpvc from v_ubicaciones v where vc.codubi = v.codubi) DATAC_IDPROV_DIRCLI,
       (SELECT coddst FROM v_ubicaciones v WHERE vc.codubi = v.codubi) DATAC_IDDIST_DIRCLI,
       (SELECT nomest FROM v_ubicaciones v WHERE vc.codubi = v.codubi) DATAV_DEPA_DIRCLI,
       (select nompvc from v_ubicaciones v where vc.codubi = v.codubi) DATAV_PROV_DIRCLI,         
       (SELECT nomdst FROM v_ubicaciones v WHERE vc.codubi = v.codubi) DATAV_DIST_DIRCLI, 
      ...
      (select vg.descripcion from VTATABGEOREF VG where vg.idplano = vc.IDPLANO and vg.codubi = vc.CODUBI) DATAV_PLANODIR,
       ...
       (select codest from v_ubicaciones v where v.codubi = vs.ubisuc) DATAV_IDDEPI,
       (select codpvc from v_ubicaciones v where v.codubi = vs.ubisuc) DATAC_IDPROVI,
       (select coddst from v_ubicaciones v where v.codubi = vs.ubisuc) DATAC_IDDISTI,
       (select nomest from v_ubicaciones v where v.codubi = vs.ubisuc) DATAV_DEPARTAMENTOI,
       (select nompvc from v_ubicaciones v where v.codubi = vs.ubisuc) DATAV_PROVINCIAI,
       (select nomdst from v_ubicaciones v where v.codubi = vs.ubisuc) DATAV_DISTRITOI,
