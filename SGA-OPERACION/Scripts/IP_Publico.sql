...
select vc.tipper DATAC_TIPO_PERSONA,
      ...
      (select tc.descripcion from tipo_calificacion tc, vtatabslcfac v 
       ...
       case
         when nvl(ipo.flgprinc, 0) = 0 and
             lower(ipb.descripcion) like '%ip%blic%' then
          ipb.descripcion
       end DATAV_DESCPLAN,
      'IP Público' DATAV_TIPOSERVICIO,
      ...
      (select abrdid from vtatipdid 
        where tipdide = (select vt.tipdide 
                           from vtatabslcfac v, VTATABECT vt 
                          where v.numslc = (select numslc from solot
                                             where codsolot = 
                                                 (select max(s.codsolot)
                                                    from solot s, solotpto pto
...
where  
 ...
 ( trunc(nvl(ipb.fecfin,'01/01/1900')) >= trunc(sysdate)
   ...
   and ipb.estado = 1
   and lower(ipb.descripcion) like '%ip p%blic%'
