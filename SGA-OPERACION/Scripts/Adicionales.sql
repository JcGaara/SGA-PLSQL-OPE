...
select vc.tipper DATAC_TIPO_PERSONA,
       ...
       case
         when nvl(ipo.flgprinc, 0) = 0 and ipo.codequcom is null then
          ipb.descripcion
       end DATAV_DESCPLAN,
      'Adicionales' DATAV_TIPOSERVICIO,
      ...
      (select descripcion from estinssrv where  estinssrv = iso.estinssrv) DATAV_DESCESTSERV,
       ...
       (select nvl(flag_lc,0) from tystabsrv where codsrv=ipb.codsrv) DATAN_CONTROL,
       ...
       nvl(ipb.cantidad,0) DATAN_CANTIDAD,
       'No' DATAC_PUBLICAR,      
      ...
      (SELECT nvl(vp.presusc,0) FROM VTATABPRECON vp, vtatabslcfac v 
        WHERE v.numslc = vp.numslc 
          and v.numslc = (select numslc from solot
                           where codsolot = 
                               (select max(s.codsolot)
      ...
      (SELECT nvl(vp.publicar,0) FROM VTATABPRECON vp, vtatabslcfac v 
        WHERE v.numslc = vp.numslc 
          and v.numslc = (select numslc from solot
                           where codsolot = 
                               (select max(s.codsolot)
...
where  
 ...
 ( trunc(nvl(ipb.fecfin,'01/01/1900')) >= trunc(sysdate)
   ...
   and nvl(ipo.flgprinc, 0) = 0
   and lower(ipb.descripcion) not like '%puerto 25%'
   and lower(ipb.descripcion) not like '%ip p%blic%'
