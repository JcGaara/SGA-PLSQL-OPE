CREATE OR REPLACE PROCEDURE OPERACION.P_CREAR_SOLOT_X_ANULA(a_codsolot IN NUMBER) IS
  /**********************************************************************
  Genera la copia del SOT que pasa a estado RECHAZADO
  20070815 Roy Concepcion Creación del procedimiento
  20071217 Sergio Le Roux relacion padre-hijo de solot
  **********************************************************************/
  l_codsolot    NUMBER;
...
BEGIN
  ...
  INSERT INTO SOLOTANULA
    (CODSOLOT, CODPADRE)
  VALUES
    (l_codsolot, a_codsolot);

  SELECT MAX(codsolot) INTO l_codsolotpto FROM solot;

 /* INSERT INTO TMP_SOLOT_CODIGO
    (CODSOLOT, ESTADO, USUARIO, FECHAREGISTRO, FECHAEJECUCION)
  VALUES
    (l_codsolotpto, 0, user, sysdate, sysdate);*/

  DECLARE

    CURSOR r_detpto IS
      SELECT l_codsolotpto codsolot,
             tiptrs,
             codsrvant,
             bwant,
             codsrvnue,
