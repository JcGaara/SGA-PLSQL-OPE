CREATE OR REPLACE TRIGGER OPERACION.SGATRI_SGAT_USU_DW
BEFORE INSERT ON OPERACION.SGAT_USU_DW
...
BEGIN
  ...
  l_transac   := 'INSERT PK:'||:new.USUDV_USUARIO;
  ...
  l_msgerror  :='';
	...
	FOR c IN c_temp LOOP
		...
		IF (c.COLUMN_NAME = 'USUDV_USUARIO') THEN
		...
		IF (c.COLUMN_NAME = 'USUDN_ACTIVO') THEN
			AUDITORIA.PKG_log_auditoria.SGASI_LOG_AUDIT(l_evento, l_nomevento, 
				OPERACION.PKG_SIGCORP.SGAFUN_USUARIOOPE(USER), l_modulo, c.OWNER,
